...
const DetailPage: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      {/* Header Card */}
      <div className="bg-slate-850 rounded-2xl p-8 border border-slate-700 shadow-xl mb-12 flex flex-col md:flex-row gap-8 items-start">
        <div className="w-24 h-24 md:w-32 md:h-32 rounded-2xl bg-slate-700 overflow-hidden border-4 border-slate-600 flex-shrink-0">
        ...
        <div className="flex-1">
          <div className="flex flex-wrap items-center gap-4 mb-4">
             ...
             <div className="flex items-center gap-1 bg-yellow-500/20 text-yellow-400 px-3 py-1 rounded-full font-bold">
                 <span>★</span>
          ...
          <div className="flex flex-wrap gap-2">
            {provider.tags.map(tag => (
              <span key={tag} className="px-3 py-1 bg-slate-800 text-primary-300 text-sm rounded-full border border-slate-700">
                #{tag}
              </span>
            ))}
          </div>
        ...
        <div className="flex-shrink-0 w-full md:w-auto flex flex-col gap-4">
           ...
           <div className="grid grid-cols-2 gap-4 text-center">
             <div className="bg-slate-900 p-3 rounded-lg border border-slate-700">
               <div className="text-xs text-slate-500">速度得分</div>
               <div className="text-xl font-bold text-indigo-400">{provider.speedScore}</div>
             </div>
             <div className="bg-slate-900 p-3 rounded-lg border border-slate-700">
               <div className="text-xs text-slate-500">稳定性</div>
               <div className="text-xl font-bold text-emerald-400">{provider.stabilityScore}</div>
             </div>
           </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-12">
          ...
          <section>
            ...
            <div className="bg-slate-850 p-6 rounded-xl border border-slate-700 h-80">
            ...
            <p className="text-xs text-slate-500 mt-2 text-center">* 数据基于千兆宽带环境测试，实际体验可能因地区和运营商而异。</p>
          </section>
                ...
                {provider.reviews.map((review, idx) => (
                  <div key={idx} className="bg-slate-900 p-6 rounded-xl border border-slate-800">
                    <div className="flex justify-between items-start mb-2">
                       <div className="font-bold text-slate-200">{review.user}</div>
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-24 space-y-8">
            <section>
              <h2 className="text-2xl font-bold text-white mb-6">套餐价格</h2>
              <div className="space-y-4">
                {provider.plans?.map((plan, idx) => (
                  <div key={idx} className="bg-slate-850 rounded-xl border border-slate-700 overflow-hidden hover:border-primary-500/50 transition-all">
                     ...
                     <div className="p-5">
                       <div className="text-sm font-medium text-slate-300 mb-3">包含 {plan.traffic} 流量</div>
                ...
                {(!provider.plans || provider.plans.length === 0) && (
                  <div className="text-slate-500 text-sm text-center p-4">暂无详细套餐数据</div>
                )}
              </div>
            </section>

             <section className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl p-6 border border-slate-700">
                <h3 className="font-bold text-white mb-2">CloudFly 简评</h3>
